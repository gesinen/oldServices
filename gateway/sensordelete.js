		...
		if(parseData.originalRequest.method == "DELETE")
			{
			 ...
			 queryUpdate="update swat_gesinen.sensor_gateway_pkid set pk_id="+ null +" where mac_number ='"+ macNumber  +"' and  sensor_id=" +  parseData.originalRequest.requestId;
			//console.log(response);		

	//console.log(patharray[1]);
	
	db.getConnection(function(err,connection){	
	connection.query("select * from swat_gesinen.sensor_info where id=" +  parseData.originalRequest.requestId ,function(err,rows){	
	connection.release();
	if(rows.length > 0) {
		db.getConnection(function(err,connection){			

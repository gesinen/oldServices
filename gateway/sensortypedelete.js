		...
		if(parseData.originalRequest.method == "DELETE")
			{
			 ...
			 queryDelete="delete from swat_gesinen.sensor_type_gateway_pkid  where mac_number ='"+ macNumber  +"' and  sensor_type_id=" +  parseData.originalRequest.requestId;
			//console.log(response);		

	//console.log(patharray[1]);
	
	db.getConnection(function(err,connection){	
	connection.query("select * from swat_gesinen.sensor_type_info where id=" +  parseData.originalRequest.requestId ,function(err,rows){	
	connection.release();
	if(rows.length > 0) {
		db.getConnection(function(err,connection){			

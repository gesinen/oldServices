module.exports = function(client,data) { 
	...
	// console the data recored array as string
	...
	db.getConnection(function(err,connection){
		...
		var activePower = error == 1 ? 0:dataRecordArray[2]/10;
		var powerFactor = error == 1 ? 0:dataRecordArray[3]/1000;		
		var activeEnergy = error == 1 ? 0:dataRecordArray[8]/10;
		var reactiveEnergy = error == 1 ? 0:dataRecordArray[9]/10;
		...
		connection.query("insert into swat_gesinen.energy_meter_value	(device_id,counter_number,phase,volt_in_ver,current_ir,active_par,power_pfr,qat,qr_one_t,error_info,updated_dt,created_dt)values('"+ deviceId +"','"+ counterNumber +"','"+ phase +"','"+ voltage +"','"+ current +"','"+ activePower +"','"+ powerFactor +"','"+ activeEnergy +"','"+ reactiveEnergy +"','"+ error +"','"+ mydate +"','"+ mydate +"')",function(err,rows){
			...
			var voltageR = error == 1 ? 0:dataRecordArray[0]/10;
			var voltageS = error == 1 ? 0:dataRecordArray[1]/10;
			var voltageT = error == 1 ? 0:dataRecordArray[2]/10;
			var currentR = error == 1 ? 0:dataRecordArray[3]/1000;
			var currentS = error == 1 ? 0:dataRecordArray[4]/1000;
			var currentT = error == 1 ? 0:dataRecordArray[5]/1000;			
			var activePowerR = error == 1 ? 0:dataRecordArray[6]/10;
			var activePowerS = error == 1 ? 0:dataRecordArray[7]/10;
			var activePowerT = error == 1 ? 0:dataRecordArray[8]/10;			
			var powerFactorR = error == 1 ? 0:dataRecordArray[9]/1000;
			var powerFactorS = error == 1 ? 0:dataRecordArray[10]/1000;
			var powerFactorT = error == 1 ? 0:dataRecordArray[11]/1000;			
			var activeEnergy = error == 1 ? 0:dataRecordArray[16]/10;
			var reactiveEnergy = error == 1 ? 0:dataRecordArray[17]/10;

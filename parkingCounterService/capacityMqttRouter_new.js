...
async function getCurrentCapacityAndParkingId(deviceEui) {
    let sqlGetCurrentCapacity = "SELECT capacity_parking.currentCapacity, capacity_parking.maxCapacity, capacity_parking.id as parkingId, capacity_devices.type as parkingType FROM `sensor_info` INNER JOIN capacity_devices ON " +
...
async function getCurrentCapacityAndMaxCapacityPakingIdAndType(deviceEui) {
    let sqlGetCurrentCapacity = "SELECT capacity_parking.currentCapacity, capacity_parking.maxCapacity, capacity_parking.id as parkingId, capacity_devices.type as parkingType FROM `sensor_info` INNER JOIN capacity_devices ON " +
        "capacity_devices.sensorId=sensor_info.id INNER JOIN capacity_type_spot ON " +
        "capacity_type_spot.capacityDeviceId=capacity_devices.id INNER JOIN capacity_parking ON" +
        " capacity_parking.id=capacity_type_spot.parkingId WHERE sensor_info.device_EUI = '" + deviceEui + "';"
    let res = await query(sqlGetCurrentCapacity)
    console.log("res", res)
	if(res.length > 0){
    return {
...
async function saveMessageLog(messageHex, frameCounter, capacityDeviceEUI, parkingId, currentCapacity, maxCapacity) {
    ...
    const actualCurrentCapacityVal = currentCapacity + parseInt(entrada - salida + bidir1 - bidir2);
	const actualCurrentCapacity = actualCurrentCapacityVal  > (maxCapacity + 3) ? (maxCapacity + 3) : actualCurrentCapacityVal;
...
// for individual parking save the log
async function saveMessageLogForParkoid(messageHex, frameCounter, binaryString,capacityDeviceEUI, parkingId, currentCapacity, maxCapacity) {
    
    sqlInsertLogOnTable = "INSERT INTO `capacity_devices_log` (`messageHex`, `frameCounter`, `messageInt`,`capacityDeviceEUI`, `parkingId`, `currentCapacity`, `maxCapacity`) VALUES " +
        "('" + messageHex + "', " + frameCounter + ", '"+binaryString+"', '" + capacityDeviceEUI + "', " + parkingId + ", " + currentCapacity + ", " + maxCapacity + ");"

    console.log("sqlInsertLogOnTable parkoid", sqlInsertLogOnTable);
...
async function getUsernameByParking(parkingId) {
    let sqlGetCurrentCapacity = "SELECT users.email AS username, users.id as userId FROM users INNER JOIN capacity_parking ON users.id = capacity_parking.userId WHERE capacity_parking.id = " + parkingId + ";"
    let res = await query(sqlGetCurrentCapacity)
    console.log("res", res)
    return {
...
async function main() {
    ...
    client.on('message', async function(topic, message) {
        if (topic != undefined) {
            ...
            if (splitedTopic[1] == "application" && (splitedTopic[2] == "2" || splitedTopic[2] == "4") && splitedTopic[3] == "device" && splitedTopic[5] == "rx") {
				...
				//this if for parkoid sensorsors 
				if (messageFormated.object.DecodeDataString != "PING" && messageFormated.fPort == 3) {
					// Obtengo cual parking esta asociado a este sensor
                    let currentCapacityAndParkingId = await getCurrentCapacityAndMaxCapacityPakingIdAndType(deviceEUI);
					if(currentCapacityAndParkingId.parkingType == "parking_individual_Parkdroid"){
						console.log('parkingType',currentCapacityAndParkingId.parkingType);
						...
						console.log('binaryString',binaryString);
						 ...
						 console.log('binaryarray',binaryarray);
						  let statusOfPaking = binaryarray[7] == '0' ? 0 :1;
						   
						  console.log('statusOfPaking',statusOfPaking);

                    await saveMessageLogForParkoid(messageFormated.object.DecodeDataHex, messageFormated.fCnt, binaryString,deviceEUI, currentCapacityAndParkingId.parkingId,
                        statusOfPaking, currentCapacityAndParkingId.maxCapacity)

                    console.log('save the log of individual parking')
					}
				}
                ...
                if (messageFormated.object.DecodeDataString != "PING" && messageFormated.fPort == 1) {
                    ...
                    if(username.includes('informatica@montserrat.es')) messageWindow = "06"
					else if(username.includes('montserrat')) messageWindow = "06"
                    ...
                    for (const element of cartelDeviceEUIandLinesArray) {
                        ...
                        if (element[0].parkingId != null && element[1].parkingId != null) {
                            ...
                            if (element[0].parkingId == currentCapacityAndParkingId.parkingId) {
								...
								let capacityCalculated = parseInt(element[0].currentCapacity)+ parseInt(currentCapacityFromMsg) > parseInt(element[0].maxCapacity) + 3 ? parseInt(element[0].maxCapacity) + 3 : parseInt(element[0].currentCapacity)+ parseInt(currentCapacityFromMsg);
								...
								if(username.includes('xirivella')){
									let xirivellaSpotString = '0';
									if(currentCapacityAndParkingId.parkingType == "parking_individual_Parkdroid"){
								
							let xirivellaSpot  = await getParkingIdOfParkingIndividualParkoid(userId);
							 let freeSpots  = 1 - xirivellaSpot.currentXirivellaSpot;
							 console.log('freeSpots',freeSpots);
							
							 xirivellaSpotString = String(freeSpots).padStart(2, '0');
									console.log('xirivellaSpotString',xirivellaSpotString);
									}
							 
                                //const xirivellaSpot = await getXirivellaSpecialSpot(element[0].parkingId)
                                //let xirivellaSpotString;
                                //if(xirivellaSpot.currentXirivellaSpot != undefined) xirivellaSpotString = xirivellaSpot.currentXirivellaSpot
                                //else xirivellaSpotString = "00"
                                setMessageHex = "1B " + messageWindow + " " + intToHex(capacityFreeSpaces) + " "+ capacityFreeSpacesOther;//xirivellaSpotString;
                            }
							else{	
                                let setMessageHex = "1B " + messageWindow + " " + ('0'+intToHex(capacityFreeSpaces)).slice(-2) + " 00 " + ('0'+intToHex(capacityFreeSpacesOther)).slice(-2) + " 00"
							}
								...
								let capacityCalculated = parseInt(element[1].currentCapacity)+ parseInt(currentCapacityFromMsg) > parseInt(element[1].maxCapacity) + 3 ? parseInt(element[1].maxCapacity) + 3 : parseInt(element[1].currentCapacity)+ parseInt(currentCapacityFromMsg);
                            ...
                            setMessageHex = "1B " + messageWindow + " " + intToHex(capacityFreeSpaces) + " 0 0 0"
                            let setMessage = hexToBase64(setMessageHex)
                            let sqlQueryUpdateParkingCapacity = "UPDATE capacity_parking SET `currentCapacity`=" +
                                capacityCalculated + " WHERE id=" + cartelDeviceEUIandLinesArray[index][0].parkingId + ";"
...
async function getXirivellaSpecialSpot(parkingId){
    ...
    let returnResponse;
    if(res.length > 0) returnResponse = res[0].status
    else returnResponse = undefined
    return {
        currentXirivellaSpot: returnResponse
    }
}
async function getParkingIdOfParkingIndividualParkoid(userId){
    let sqlGetCurrentCapacity = "select capacity_devices.id as deviceId, capacity_parking.id as ParkingId,capacity_type_spot.status as status  from capacity_devices Inner join capacity_type_spot On capacity_devices.id = capacity_type_spot.capacityDeviceId Inner Join capacity_parking On capacity_type_spot.parkingId = capacity_parking.id where capacity_devices.userId = "+userId+" and capacity_devices.type ='parking_individual_Parkdroid'";
    let res = await query(sqlGetCurrentCapacity)
    console.log("responseFromXirivellaCapacity", res,res.length,res[0].status)
    let returnResponse;
    if(res.length > 0){ returnResponse = res[0].status}
    else {returnResponse = undefined}
    return {
